...
new_topic_temperature_alerts = NewTopic(
    name=topic_temperature_alerts,
    num_partitions=kafka_topic_config["num_partions"],
    replication_factor=kafka_topic_config["replication_factor"],
)

new_topic_humidity_alerts = NewTopic(
    ...
    try:
        admin_client.create_topics(
            new_topics=[
                ...
                new_topic_temperature_alerts,
                new_topic_humidity_alerts,
            ],
            validate_only=False,
        )
        print(
            f"{GREEN}Topics {LIGHT_GREEN}{topic_building_sensors}, {topic_temperature_alerts}, {topic_humidity_alerts}{END} {GREEN}created successfully.{END}"
        )
